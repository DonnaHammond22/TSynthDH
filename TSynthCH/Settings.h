...
void settingsEFXselEnable(char * value);
...
//DH adding setting settingsEFXsel(char * value);

FLASHMEM void settingsEFXselEnable(char * value){
	if (strcmp(value, "Off") == 0) {
    EFXsel = false;
    storeEFXselEnable(0);
  } else {
...
FLASHMEM int currentIndexEFXselEnable() {
	return getEFXselEnable() ? 1 : 0;
}
...
FLASHMEM void setUpSettings() {
  ...
  settingsOptions.push(SettingsOption{"EFXsel", {"Off", "On", '\0'}, settingsEFXselEnable, currentIndexEFXselEnable});
}

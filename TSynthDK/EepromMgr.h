...
FLASHMEM boolean getEFXselEnable() {
  byte efxsel = EEPROM.read(EEPROM_EFXsel_ENABLE); 
  if (efxsel < 0 || efxsel > 1)return false; //If EEPROM has no efxsel enable stored
  return efxsel == 1 ? true : false;
}

FLASHMEM void storeEFXselEnable(byte EFXselEnable){
  EEPROM.update(EEPROM_EFXsel_ENABLE, EFXselEnable);
}

...
void Oscilloscope::Display() {
  ...
  //DH locating Scope trace depending on Screen type display 
  /*
    if (ST77XXDisplayType == 1){
      int16_t ScopeoffX = 0;
      int16_t ScopeoffY = 0;
    }
    else if (ST77XXDisplayType > 1){
      int16_t ScopeoffX = 0;
      int16_t ScopeoffY = 120;
    }
  */
  int16_t ScopeoffX = 60;
  int16_t ScopeoffY = 110;
  ...
  for (uint8_t i = 0; i < AUDIO_BLOCK_SAMPLES - 1; i++) {
    ...
    display->drawLine((pixel_x + 15 + ScopeoffX), (prev_pixel_y + ScopeoffY), (pixel_x + 16 + ScopeoffX), (pixel_y + ScopeoffY), 0x07B0);

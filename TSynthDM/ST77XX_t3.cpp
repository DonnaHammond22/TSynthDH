  ...
  _cs(cs), _rs(rs),  _rst(rst), _sid(sid), _sclk(sclk)
{
  ...
  _dma_state = 0;
    #endif
	//DH making entry for DisplayType in CG
	if(ST77XXDisplayType == 1){
		_screenHeight = ST7735_TFTHEIGHT_160;
		_screenWidth = ST7735_TFTWIDTH;
	}
	else if(ST77XXDisplayType == 2){
		_screenHeight = ST7789_TFTHEIGHT2;
		//_screenHeight = 240;
		_screenWidth = ST7789_TFTWIDTH2;
		//_screenWidth = 240;
	}
	else if(ST77XXDisplayType == 3){
		_screenHeight = ST7789_TFTHEIGHT3;
		//_screenHeight = 240;
		_screenWidth = ST7789_TFTWIDTH3;
		//_screenWidth = 240;
	}
	else if(ST77XXDisplayType == 4){
		_screenHeight = ST7789_TFTHEIGHT4;
		//_screenHeight = 320;
		_screenWidth = ST7789_TFTWIDTH4;
		//_screenWidth = 240;
	}
  //_screenHeight = ST7735_TFTHEIGHT_160;
  //_screenWidth = ST7735_TFTWIDTH; 
...
// DH inputing 7789 init
void  ST7735_t3::init(uint16_t width, uint16_t height, uint8_t mode)
{
...
uint8_t ST7735_t3::useFrameBuffer(boolean b)    // use the frame buffer?  First call will allocate
{
  if (b) {
    ...
    if (_pfbtft == NULL) {
     ...
     // DH forcing value for _count_pixels, this makes it work !!
      _count_pixels = (240*240);
